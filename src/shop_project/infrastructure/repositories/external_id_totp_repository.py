from sqlalchemy import select

from shop_project.application.dto.external_id_totp_dto import ExternalIdTotpDTO
from shop_project.application.dto.mapper import to_domain
from shop_project.infrastructure.database.models.external_id_totp import (
    ExternalIdTotp as ExternalIdTotpORM,
)
from shop_project.infrastructure.entities.external_id_totp import ExternalIdTotp
from shop_project.infrastructure.query.base_query import BaseQuery
from shop_project.infrastructure.query.composed_query import ComposedQuery
from shop_project.infrastructure.query.custom_query import CustomQuery
from shop_project.infrastructure.repositories.base_repository import BaseRepository


class ExternalIdTotpRepository(BaseRepository[ExternalIdTotp, ExternalIdTotpDTO]):
    model_type = ExternalIdTotp
    dto_type = ExternalIdTotpDTO

    async def create(self, items: list[ExternalIdTotpDTO]) -> None:
        if not items:
            return

        for dto in items:
            entity = ExternalIdTotpORM(**dto.model_dump())
            self.session.add(entity)

    async def update(self, items: list[ExternalIdTotpDTO]) -> None:
        if not items:
            return

        for dto in items:
            entity = self.session.identity_map.get(
                self._get_identity_key(ExternalIdTotpORM, dto.entity_id)
            )
            if not entity:
                continue

            entity.repopulate(**dto.model_dump())

    async def delete(self, items: list[ExternalIdTotpDTO]) -> None:
        if not items:
            return

        for dto in items:
            entity = self.session.identity_map.get(
                self._get_identity_key(ExternalIdTotpORM, dto.entity_id)
            )

            if not entity:
                raise RuntimeError(
                    f"Entity of type {self.model_type.__name__} {dto.entity_id} not found for deleting"
                )

            await self.session.delete(entity)

    async def load(self, query: BaseQuery) -> list[ExternalIdTotp]:
        if isinstance(query, ComposedQuery):
            stmt = select(ExternalIdTotpORM).where(
                query.criteria.to_sqlalchemy(ExternalIdTotpORM)
            )
            stmt = self._apply_lock(stmt, query.lock, [ExternalIdTotpORM])

        elif isinstance(query, CustomQuery):
            stmt = query.compile_sqlalchemy()

        else:
            raise ValueError(f"Unknown query type: {type(query)}")

        rows = await self.session.execute(stmt)
        orm_entities = rows.scalars().unique().all()

        res = [to_domain(self.dto_type.model_validate(e)) for e in orm_entities]

        return res
