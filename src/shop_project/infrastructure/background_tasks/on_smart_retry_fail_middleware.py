...
__all__ = ("OnSmartRetryFailMiddleware",)

_logger = getLogger("taskiq.smart_retry_middleware")


class OnSmartRetryFailMiddleware(TaskiqMiddleware):
    ...
    def __init__(
        ...
        on_out_of_retries_callback: (
            Callable[[TaskiqMessage, BaseException], Awaitable[None]] | None
        ) = None,
    ) -> None:
        """
        ...
        :param on_fail_callback: Callback to execute when task reaches out of retries.
        """
        super().__init__()
        ...
        self.on_out_of_retries_callback = on_out_of_retries_callback
    ...
    async def on_error(
        ...
        if retries < max_retries:
            ...
            if self.on_out_of_retries_callback is not None:
                await self.on_out_of_retries_callback(message, exception)
