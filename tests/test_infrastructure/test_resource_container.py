...
def test_get_by_id(purchase_draft_factory: Callable[[], PurchaseDraft]) -> None:
    ...
    assert (
        container.get_by_id(PurchaseDraft, purchase_draft.entity_id) == purchase_draft
    )
...
def test_get_by_attribute(purchase_draft_factory: Callable[[], PurchaseDraft]) -> None:
    ...
    assert container.get_by_attribute(
        PurchaseDraft,
        "entity_id",
        [purchase_draft_1.entity_id, purchase_draft_2.entity_id],
    ) == [purchase_draft_1, purchase_draft_2]
    assert container.get_by_attribute(
        PurchaseDraft, "customer_id", [purchase_draft_1.customer_id]
    ) == [purchase_draft_1]


def test_get_by_wrong_attribute(
    ...
    with pytest.raises(AttributeError):
        container.get_by_attribute(
            PurchaseDraft, "wrong_attribute", [purchase_draft_1.entity_id]
        )
...
def test_snapshot_create(purchase_draft_factory: Callable[[], PurchaseDraft]) -> None:
    ...
    assert (
        to_dto(purchase_draft)
        in container.get_resource_changes()[PurchaseDraft]["CREATED"]
    )
...
def test_snapshot_delete(purchase_draft_factory: Callable[[], PurchaseDraft]) -> None:
    ...
    assert (
        to_dto(purchase_draft)
        in container.get_resource_changes()[PurchaseDraft]["DELETED"]
    )
...
def test_snapshot_update(purchase_draft_factory: Callable[[], PurchaseDraft]) -> None:
    ...
    assert (
        to_dto(purchase_draft)
        in container.get_resource_changes()[PurchaseDraft]["UPDATED"]
    )


def test_snapshot_no_previous(
    purchase_draft_factory: Callable[[], PurchaseDraft],
) -> None:
    container = ResourceContainer(resources_registry=ResourcesRegistry.get_map())
    purchase_draft_1 = purchase_draft_factory()
