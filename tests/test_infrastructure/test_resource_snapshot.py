...
def test_intersect(purchase_draft_factory: Callable[[], PurchaseDraft]):
    purchase_draft_1 = purchase_draft_factory()
    purchase_draft_2 = purchase_draft_factory()
    snapshot_before: ResourceSnapshot = get_resource_snapshot({PurchaseDraft: [purchase_draft_1, purchase_draft_2]})
    
    purchase_draft_1_snapshot = to_dto(purchase_draft_1)
    
    purchase_draft_1.finalize()
    
    snapshot_after: ResourceSnapshot = get_resource_snapshot({PurchaseDraft: [purchase_draft_1, purchase_draft_2]})
    ...
    assert intersect_by_id_before_side.to_dict()[PurchaseDraft] == [purchase_draft_1_snapshot, purchase_draft_2_snapshot]
    
    intersect_by_id_after_side = snapshot_after.intersect_identity(snapshot_before)
    assert intersect_by_id_after_side.to_dict()[PurchaseDraft] == [to_dto(purchase_draft_1), purchase_draft_2_snapshot]
    
    intersect_by_content_before_side = snapshot_before.intersect_content(snapshot_after)
    assert intersect_by_content_before_side.to_dict()[PurchaseDraft] == [purchase_draft_2_snapshot]
    
    intersect_by_content_after_side = snapshot_after.intersect_content(snapshot_before)
    assert intersect_by_content_after_side.to_dict()[PurchaseDraft] == [purchase_draft_2_snapshot]
...
def test_difference(purchase_draft_factory: Callable[[], PurchaseDraft]):
    ...
    difference_by_content_before_side = snapshot_before.difference_content(snapshot_after)
    assert difference_by_content_before_side.to_dict()[PurchaseDraft] == [purchase_draft_1_snapshot]
    
    difference_by_content_after_side = snapshot_after.difference_content(snapshot_before)
    assert difference_by_content_after_side.to_dict()[PurchaseDraft] == [to_dto(purchase_draft_1)]
